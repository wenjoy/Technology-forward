const crypto = require('crypto');
const fs = require('fs');
const publicKey = `-----BEGIN PUBLIC KEY-----

mQIDAQAB
-----END PUBLIC KEY-----`

const privateKey = `-----BEGIN RSA PRIVATE KEY-----

oH8q9GoobUD0CLJPy6sF9Dc4UhAbP+eBcMCpM7N6+5dV2gmapSC7
-----END RSA PRIVATE KEY-----`

// case 1: using crypto to encrypt and decrypt
const result = crypto.publicEncrypt(publicKey, Buffer.from('321'))
console.log('cipher', result)
const plain = crypto.privateDecrypt(keyObject, result)
console.log('plain: ', plain);

//case 2: using jsencrypt encrypt then using crypto encrypt
// just using the result directly, usage of jsencrypt see the repo's readme
 
const result1 = fs.readFileSync('./cipher');
const keyObject = crypto.createPrivateKey(privateKey);
console.log('keyObject: ', keyObject);
//NOTES: must specify padding, because using pkcs#1v1_5 default
// but crypto use pkcs#1_aoep, which will cause
/**
Error: error:04099079:rsa routines:RSA_padding_check_PKCS1_OAEP_mgf1:oaep decoding error
keyObject.padding = crypto.constants.RSA_PKCS1_PADDING;
 */
const plain = crypto.privateDecrypt(keyObject, result1)
console.log('plain: ', plain);

